...
public class TutorExplore extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getCourses(token);
        binding.collapsibleText.setText("explore your courses");
    ...
    private void getCourses(String token){
        Call<Courses> caller = retroInterface.getMyCourse("Bearer " + token);
        caller.enqueue(new Callback<Courses>() {
            @Override
            public void onResponse(Call<Courses> call, Response<Courses> response) {
                if(response.isSuccessful()) {
                    TutorExplore.this.courses = response.body().data;
                }
            }
